...
$(document).ready(function () {
    ...
    // Preencher o formulário com os dados do objeto, se disponível
    if (obj) {
        ...
        $('#formCadastro #CPF').val(obj.CPF); // Adiciona o CPF
        IdCliente = obj.Id;
        loadBeneficiarios();
    }
    ...
    function loadBeneficiarios() {
        // Verifica se a variável beneficiarios está vazia
        if (beneficiarios.length === 0) {
            $.ajax({
                url: urlBeneficiarioList + "?idCliente=" + IdCliente + "&jtStartIndex=" + (currentPage - 1) * pageSize + "&jtPageSize=" + pageSize + "&jtSorting=Nome ASC",
                method: 'POST',
                success: function (data) {
                    beneficiarios = data.Records;
            ...
            // Se beneficiarios não estiver vazio, apenas renderiza a tabela
            renderBeneficiariosTable();
        }
    }
